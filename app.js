        ...
        if (index % 2 === 0) {
          document.getElementById(
            "game-projects-container"
               ...
               <button onclick="window.game_project_${index}_dialog.showModal()" class="learn-more">Learn more about ${project.name}</button></div>
                 </div>
                 
                 <dialog id="game_project_${index}_dialog" class="dialog">
                <button onclick="window.game_project_${index}_dialog.close()" class="close-button" id="close-${index}">X</button>
                 <div class="dialog-content">
                 ${url}
                    <h2>${project.name}</h2>
                    
                    <p>${project.description}</p>
                    <a href="${project.button_link}" target="_blank" class="learn-more">${project.button_text}</a>
...
<button onclick="window.game_project_${index}_dialog.showModal()" class="learn-more">Learn more about ${project.name}</button></div>
      ...
      data.writing_projects.map((project, index) => {
        let url = "";
        if (project.photo_type == "image") {
          url = `<img class="project-image" src="${project.photo_url}" alt="${project.name}"></img>`;
        } else if (project.photo_type == "video") {
